...
# individually from the command line (e.g. `--ode bernoulli`). Leave integration
# fields as None to fall back via resolve_ode_args to ODE_DEFAULTS[ode]; leave
# param_range as None (with param_list None) to fall back via resolve_basis_args.
DEFAULT_CASE = dict(
    ode="bernoulli",
    basis_type="monomial",
    param_range=(0, 1),
...
def main():
    ...
    # 4. Plot singular value spectrum empty circle markers
    fig, ax = plt.subplots(1,2)
    ax[0].semilogy(S/S[0],marker='o', linestyle='None')
    ax[0].set_xlabel("Index $i$")
    ax[0].set_ylabel("$\sigma_i / \sigma_1$")
    ...
    fig2, ax2 = plt.subplots(1, 2, subplot_kw={'projection': '3d'}, figsize=(12, 6))
    db.scaled_3D_quiver(X_stacked, V, mode="tangent", ax=ax2[0])
    ax2[0].set_title("Reconstructed infinitesimal generator (jet space)")
    db.scaled_3D_quiver(X_stacked, N, mode="normal", ax=ax2[1])
    ax2[1].set_title("Analytic normal field")
    fig2.tight_layout()
